...
import cn.ppps.forwarder.databinding.FragmentClientCallQueryBinding
...
class CallQueryFragment : BaseFragment<FragmentClientCallQueryBinding?>() {
    ...
    private fun loadRemoteData(refresh: Boolean) {
        ...
        val queryData = CallQueryData(callType, pageNum, pageSize, keyword)

        RelayApi.request(RelayCommands.CMD_CALL_QUERY, RelayClientHolder.gson.toJson(queryData), RelayCommands.RSP_CALL_QUERY, { json ->
            try {
                val resp = RelayClientHolder.gson.fromJson<BaseResponse<List<CallInfo>?>>(json, object : TypeToken<BaseResponse<List<CallInfo>?>>() {}.type)
                if (resp.code == 200) {
                    pageNum++
                    if (refresh) {
